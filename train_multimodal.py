    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(train_progress):
            ...
            try:
                ...
                # NOTE: Paired labels are now handled INSIDE the model's forward pass
                # The model will double the outputs when original_video_frames is present
                # So we need to double the labels HERE to match the model's doubled outputs
                has_originals = 'original_video_frames' in batch and batch['original_video_frames'] is not None
                if has_originals and batch['original_video_frames'].shape[0] == labels.shape[0]:
                    # The model will output [fake_predictions, real_predictions]
                    # So we need labels [fake_labels, real_labels]
                    labels_paired = torch.cat([
                        labels,  # Labels for fake videos (should all be 1)
                        torch.zeros_like(labels)  # Labels for original videos (all 0)
                    ])
                    # Store original labels for batch validation
                    original_labels = labels
                    labels = labels_paired
                    if self.debug and batch_idx == 0:
                        print(f"[CONTRASTIVE] Created paired labels for model output: {labels.shape}")
                else:
                    original_labels = labels
                
                # For first few batches, reduce effective batch size to prevent hanging
                if batch_idx < 3 and original_labels.shape[0] > 10:
                    print(f"[RANK {self.local_rank}] Reducing batch size for initial batches: {original_labels.shape[0]} -> 10")
                    # Truncate all batch elements to smaller size
                    for key, value in batch.items():
                        if isinstance(value, torch.Tensor) and value.shape[0] == original_labels.shape[0]:
                            batch[key] = value[:10]
                    original_labels = batch['label']
                    # Recreate paired labels with new size
                    if has_originals:
                        labels = torch.cat([original_labels, torch.zeros_like(original_labels)])
                    else:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_progress):
                try:
                    ...
                    # CONTRASTIVE LEARNING FIX: Double labels if original videos are present
                    # This matches the training loop behavior where paired predictions are made
                    has_originals = ('original_video_frames' in batch and 
                                   batch['original_video_frames'] is not None and
                                   batch['original_video_frames'].shape[0] == original_labels.shape[0])
                    if has_originals:
                        # Create paired labels: [1,1,1,1, 0,0,0,0] for batch_size=4
                        labels = torch.cat([original_labels, torch.zeros_like(original_labels)])
                    else:
                        labels = original_labels
