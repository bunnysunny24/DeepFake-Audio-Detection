...
try:
    ...
    if globals().get('DEBUG_MODE', False):
        print("[DEBUG] Warning: Some optional dependencies (dlib, facenet_pytorch) not found, some features may be limited")
...
try:
    ...
    if globals().get('DEBUG_MODE', False):
        print(f"[DEBUG] advanced_model_components.py not found or has errors: {e}")
        print("[DEBUG] Falling back to standard components")
...
try:
    ...
    if globals().get('DEBUG_MODE', False):
        print("[DEBUG] SkinColorAnalyzer not found, using fallback implementation (fallbacks moved to fallbacks.py)")
...
try:
    ...
    if globals().get('DEBUG_MODE', False):
        print(f"[DEBUG] voice_stress_analyzer.py not found or has errors: {e}")
        print("[DEBUG] Voice stress analysis will use basic signal processing only")
...
try:
    ...
    if globals().get('DEBUG_MODE', False):
        print(f"[DEBUG] mobile_sensor_analysis.py not found or has errors: {e}")
        print("[DEBUG] Mobile sensor features will be disabled")
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, visual_features, audio_features):
        ...
        # Add sequence dimension for proper attention: [B, dim] -> [B, 1, dim]
        visual_proj_seq = visual_proj.unsqueeze(1)
        audio_proj_seq = audio_proj.unsqueeze(1)
        
        # Visual attending to audio
        v_query = self.visual_query(visual_proj_seq)   # [B, 1, dim]
        a_key = self.audio_key(audio_proj_seq)          # [B, 1, dim]
        a_value = self.audio_value(audio_proj_seq)      # [B, 1, dim]
        
        # Attention weights: [B, 1, dim] x [B, dim, 1] -> [B, 1, 1] (correct per-sample)
        ...
        v_attended_a = torch.matmul(v_a_attn_weights, a_value).squeeze(1)  # [B, dim]
        
        # Audio attending to visual
        a_query = self.audio_query(audio_proj_seq)      # [B, 1, dim]
        v_key = self.visual_key(visual_proj_seq)        # [B, 1, dim]
        v_value = self.visual_value(visual_proj_seq)    # [B, 1, dim]
        ...
        a_attended_v = torch.matmul(a_v_attn_weights, v_value).squeeze(1)  # [B, dim]
...
class ForensicConsistencyModule(nn.Module):
    ...
    Optimized for memory efficiency on 8GB GPUs by:
    1. Downsampling input images to 56x56
    2. Using smaller hidden dimensions (64 instead of 256)
    3. Processing frames in smaller chunks
    """
    def __init__(self, input_dim, hidden_dim=64, debug=False):  # Reduced from 256 to 64
        super(ForensicConsistencyModule, self).__init__()
        # Downsample layer to reduce memory
        self.downsample = nn.AdaptiveAvgPool2d((56, 56))  # Reduce from 224x224 to 56x56
        self.conv1 = nn.Conv2d(input_dim, hidden_dim, kernel_size=3, stride=2, padding=1)  # stride=2 for further reduction
        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=2, padding=1)  # stride=2
    ...
    def forward(self, x):
        ...
        # MEMORY OPTIMIZATION: Downsample all frames first to reduce memory by 16x
        # From 224x224 -> 56x56 = 16x reduction in spatial memory
        x_flat = x.view(batch_size * num_frames, *x.shape[2:])  # [B*T, C, H, W]
        x_down = self.downsample(x_flat)  # [B*T, C, 56, 56]
        x = x_down.view(batch_size, num_frames, x.shape[2], 56, 56)  # [B, T, C, 56, 56]
        del x_flat, x_down  # Free memory immediately
        
        # Memory optimization: process in chunks if batch size is large
        if batch_size * num_frames > 64:  # Increased threshold since we downsampled
            chunk_size = max(1, 64 // num_frames)
            results = []
            ...
            for i in range(0, batch_size, chunk_size):
                ...
                # Feature extraction (no checkpointing needed with smaller input)
                conv1_out = F.relu(self.conv1(chunk_reshaped))
                conv2_out = F.relu(self.conv2(conv1_out))
                del conv1_out  # Free immediately
                ...
                del chunk_reshaped, chunk_result
            ...
            # Processing for smaller batches (already downsampled above)
            # Reshape for convolutions
            ...
            # Reshape back to batch, frames, features
            pooled = pooled.view(batch_size, num_frames, -1)
...
class MicroExpressionDetector(nn.Module):
    ...
    Optimized for 8GB GPUs by downsampling input frames.
    """
    def __init__(self, input_channels=3, hidden_dim=32):  # Reduced from 64 to 32
        super(MicroExpressionDetector, self).__init__()
        
        # Downsample input to 56x56 to save memory
        self.downsample = nn.AdaptiveAvgPool2d((56, 56))
        
        # 3D CNN for spatiotemporal feature extraction (smaller)
        self.conv3d = nn.Sequential(
            nn.Conv3d(input_channels, hidden_dim, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1)),
        ...
        self.temporal_lstm = nn.LSTM(
            input_size=hidden_dim*2*4*4,  # Flattened spatial features (reduced)
            hidden_size=hidden_dim*2,  # Reduced
        ...
        # Expression classifier (updated for smaller hidden_dim)
        self.classifier = nn.Sequential(
            nn.Linear(hidden_dim*2*2, hidden_dim*2),  # *2 for bidirectional, *2 for hidden
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def forward(self, face_crops):
        ...
        # MEMORY OPTIMIZATION: Downsample frames first
        # Reshape to [B*T, C, H, W] for downsampling
        x_flat = face_crops.view(batch_size * seq_len, *face_crops.shape[2:])
        x_down = self.downsample(x_flat)  # [B*T, C, 56, 56]
        x = x_down.view(batch_size, seq_len, face_crops.shape[2], 56, 56)
        del x_flat, x_down
...
class EyeAnalysisModule(nn.Module):
    ...
    def forward(self, eye_landmarks):
        ...
        # ⚠️ CHECK: If eye_landmarks are all zeros (failed extraction), warn (rate-limited)
        if torch.all(eye_landmarks == 0):
            if not hasattr(self, '_eye_landmark_warning_shown'):
                print("[CRITICAL WARNING] Eye landmarks are all zeros! Model learning from placeholder data!")
                print("[SOLUTION] Ensure dataset extracts eye_blink_features or enable face_mesh extraction")
                self._eye_landmark_warning_shown = True
...
class MultiModalDeepfakeModel(nn.Module):
    def __init__(self, num_classes=2, video_feature_dim=1024, audio_feature_dim=1024, 
        ...
        if self.enable_advanced_physiological:
            try:
                ...
                # Initialize basic physiological analyzer as fallback
                self.physiological_analyzer = RemotePhysiologicalAnalyzer(feature_dim=64)
        else:
        ...
        # 🆕 PER-MODALITY CLASSIFIERS (for 3 tampering types detection)
        # These allow detecting: 1) Audio-only fake, 2) Video-only fake, 3) Both fake
        print("[ARCHITECTURE] 🎯 Adding per-modality classifiers for tampering type detection")
        print("   - Audio-only classifier (detects fake audio)")
        print("   - Video-only classifier (detects fake video)")
        print("   - Combined classifier (detects both)")
        
        # Audio-only classifier (input: audio features)
        self.audio_only_classifier = nn.Sequential(
            nn.Linear(self.actual_audio_feature_dim, 256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        # Video-only classifier (input: video features)
        self.video_only_classifier = nn.Sequential(
            nn.Linear(self.actual_video_feature_dim, 256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        # Main combined classifier (input: fused features)
        self.classifier = nn.Sequential(
        ...
        # Explainability projection: projects variable-dim explainability features 
        # to a fixed 256-dim representation that matches the classifier's expected input.
        # Max explainability features: 3 features × max_dim + ~30 components × max_dim
        # Conservative upper bound: 4096 input dims
        self.explainability_projector = nn.Sequential(
            nn.Linear(4096, 512),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(512, combined_dim - transformer_dim)  # Output exactly fills the gap
        )
    ...
    def unfreeze_visual_layers(self, epoch):
        ...
        if epoch >= 3:  # Start unfreezing after warmup (3 epochs)
            layers_to_unfreeze = min(10 * (epoch - 2), 80)  # Unfreeze 10 layers per epoch (faster)
            for i, (name, param) in enumerate(self.visual_model.named_parameters()):
    ...
    def forward(self, inputs: Dict[str, Union[torch.Tensor, List, None]]) -> Tuple[torch.Tensor, Dict]:
        ...
        try:
            ...
            # Handle case where inputs is a raw tensor instead of dict
            if isinstance(inputs, torch.Tensor):
                # Assume it's video frames if it's a 5D tensor [B, T, C, H, W]
                if inputs.dim() == 5:
                    inputs = {'video_frames': inputs}
                # Assume it's audio if it's a 2D tensor [B, L]
                elif inputs.dim() == 2:
                    inputs = {'audio': inputs}
                # Assume it's video frames if 4D [B, C, H, W] - single frame
                elif inputs.dim() == 4:
                    inputs = {'video_frames': inputs.unsqueeze(1)}  # Add time dim
                else:
                    inputs = {'video_frames': inputs}
            ...
            # 🆕 MULTI-MODALITY SUPPORT: Handle missing inputs for audio-only/video-only detection
            # User can provide: 1) Only audio, 2) Only video, 3) Both
            if video_frames is None and audio is None:
                raise ValueError("At least one input (video_frames or audio) is required")
            ...
            if self.debug:
                print(f"[MODALITY] Video: {has_video}, Audio: {has_audio}")
                if not has_video:
                    print("[MODALITY] ⚠️ AUDIO-ONLY mode (no video provided)")
                if not has_audio:
                    print("[MODALITY] ⚠️ VIDEO-ONLY mode (no audio provided)")
            
            # Get batch dimensions from available input (video or audio)
            if video_frames is not None:
                batch_size, num_frames, C, H, W = video_frames.size()
            else:
                # Audio-only mode: infer batch size from audio
                batch_size = audio.shape[0]
                num_frames = 8  # Default frame count for dummy video
                C, H, W = 3, 224, 224  # Default video dimensions
            ...
            if has_video:
                ...
                # Audio-only mode: Create dummy video features (zeros)
                if self.debug:
                    print("[AUDIO-ONLY] Creating zero video features (no video input)")
                # Skip visual extraction, will create zero features later
                video_frames_flat = None
            ...
            # Apply ImageNet normalization for pre-trained models (skip if no video)
            if has_video:
                device = video_frames_flat.device
                ...
                # Audio-only mode: Create zero visual features
                device = audio.device
                visual_features = torch.zeros(batch_size, num_frames, self.actual_video_feature_dim, device=device)
            ...
            # 🔄 CONTRASTIVE LEARNING: Extract features from REAL (original) video if available
            original_video_features = None
            if original_video_frames is not None and original_video_frames.shape[0] == batch_size:
                if self.debug:
                    print(f"[CONTRASTIVE] Extracting features from REAL video for comparison")
                
                try:
                    # Process original (real) video through same pipeline
                    orig_frames_flat = original_video_frames.view(batch_size * num_frames, C, H, W)
                    orig_frames_flat = torch.clamp(orig_frames_flat, 0.0, 1.0)
                    
                    # Normalize with ImageNet stats
                    device = orig_frames_flat.device
                    mean = torch.tensor([0.485, 0.456, 0.406], device=device).view(1, 3, 1, 1)
                    std = torch.tensor([0.229, 0.224, 0.225], device=device).view(1, 3, 1, 1)
                    orig_normalized = (orig_frames_flat - mean) / std
                    orig_normalized = torch.clamp(orig_normalized, -10.0, 10.0)
                    
                    # Extract visual features
                    orig_visual_flat = self.visual_model(orig_normalized)
                    orig_visual = orig_visual_flat.view(batch_size, num_frames, -1)
                    
                    # Pool temporal features
                    if self.temporal_attention is not None:
                        orig_visual = self.temporal_attention(orig_visual)
                    original_video_features = torch.mean(orig_visual, dim=1)
                    original_video_features = self.video_projection(original_video_features)
                    
                    if self.debug:
                        print(f"[CONTRASTIVE] ✅ Real video features: {original_video_features.shape}")
                        
                except Exception as e:
                    if self.debug:
                        print(f"[CONTRASTIVE] ⚠️ Failed to extract real video features: {e}")
                    original_video_features = None
            ...
            if has_audio:
                ...
                # Video-only mode: Create dummy audio (zeros)
                if self.debug:
                    print("[VIDEO-ONLY] Creating zero audio features (no audio input)")
                device = video_frames.device
                audio = torch.zeros(batch_size, 16000, device=device)  # 1 second of silence
            ...
            # Initialize component_contributions early so voice stress can populate it
            component_contributions = {}
            try:
                with torch.cuda.amp.autocast(enabled=False):  # Disable autocast for audio model to prevent NaN
                    ...
                    # 🔄 CONTRASTIVE LEARNING: Extract features from REAL (original) audio if available
                    original_audio_features = None
                    if original_audio is not None and original_audio.shape[0] == batch_size:
                        if self.debug:
                            print(f"[CONTRASTIVE] Extracting features from REAL audio for comparison")
                        
                        try:
                            # Process original (real) audio through same pipeline
                            orig_audio = original_audio.float()
                            orig_max_vals = torch.abs(orig_audio).max(dim=1, keepdim=True)[0]
                            orig_max_vals = torch.clamp(orig_max_vals, min=1e-6)
                            orig_audio = orig_audio / orig_max_vals
                            orig_audio = torch.clamp(orig_audio, -1.0, 1.0)
                            
                            # Extract audio features
                            original_audio_features = self.audio_model(orig_audio)
                            original_audio_features = self.audio_projection(original_audio_features)
                            original_audio_features = torch.clamp(original_audio_features, -50.0, 50.0)
                            
                            if self.debug:
                                print(f"[CONTRASTIVE] ✅ Real audio features: {original_audio_features.shape}")
                                
                        except Exception as e:
                            if self.debug:
                                print(f"[CONTRASTIVE] ⚠️ Failed to extract real audio features: {e}")
                            original_audio_features = None
            ...
            # Voice Stress Analysis (Neural Networks) - moved here to be after audio_features is defined
            spec_features = None
            ...
            # Wire forensic consistency features into explainability pipeline
            explainability_features.append(forensic_features)
            component_contributions['forensic_consistency'] = forensic_features
            ...
            if facial_landmarks is None:
                ...
                zero_count = torch.sum(torch.all(facial_landmarks == 0, dim=-1)).item()
                total_frames = batch_size * num_frames
                if zero_count > total_frames * 0.5:  # More than 50% zeros
                    print(f"\n{'='*70}")
                    print("⚠️  CRITICAL WARNING: FACIAL LANDMARKS ARE MOSTLY ZEROS!")
                    print(f"{'='*70}")
                    print(f"Zero frames: {zero_count}/{total_frames} ({100*zero_count/total_frames:.1f}%)")
                    print("IMPACT: Model will learn from placeholder data, not real facial movements!")
                    print("SOLUTION: Check dataset_loader.py - facial landmark extraction may be failing")
                    print(f"{'='*70}\n")
            ...
            if head_pose is not None:
                ...
                # Validate head pose (only warn in debug mode to reduce log spam)
                if self.debug:
                    zero_samples = torch.sum(torch.all(head_pose == 0, dim=-1) if head_pose.dim() > 1 else (head_pose == 0)).item()
                    if zero_samples > batch_size * 0.5:
                        print(f"⚠️  WARNING: Head pose features are mostly zeros ({zero_samples}/{batch_size} samples)")
            else:
            ...
            if eye_blink_features is not None:
                ...
                # Validate eye blink features (only warn in debug mode to reduce log spam)
                if self.debug:
                    zero_samples = torch.sum(torch.all(eye_blink_features == 0, dim=-1) if eye_blink_features.dim() > 1 else (eye_blink_features == 0)).item()
                    if zero_samples > batch_size * 0.5:
                        print(f"⚠️  WARNING: Eye blink features are mostly zeros ({zero_samples}/{batch_size} samples)")
            ...
            if pulse_signal is not None:
                if self.debug:
                    ...
                    # Validate pulse signal (only warn in debug mode)
                    zero_samples = torch.sum(torch.all(pulse_signal == 0, dim=-1) if pulse_signal.dim() > 1 else (pulse_signal == 0)).item()
                    if zero_samples > batch_size * 0.5:
                        print(f"⚠️  WARNING: Pulse signal is mostly zeros ({zero_samples}/{batch_size} samples)")
            ...
            if frequency_features is not None:
                ...
                zero_samples = torch.sum(torch.all(frequency_features == 0, dim=-1) if frequency_features.dim() > 1 else (frequency_features == 0)).item()
                if zero_samples > batch_size * 0.5:
                    print(f"⚠️  WARNING: Frequency features are mostly zeros ({zero_samples}/{batch_size} samples)")
            ...
            if hasattr(self, 'enable_mobile_sensors') and self.enable_mobile_sensors:
                ...
                try:
                    ...
                    # ❌ DON'T add to advanced_features_list - causes dimension explosion (768 → 4096)
                    # Mobile features are already used via component_contributions for weighted fusion
            ...
            # Check if contrastive features are available
            contrastive_features_available = (original_video_features is not None and original_audio_features is not None)
            
            # Combine and fuse features
            if contrastive_features_available:
                ...
                # 3. Difference features (computed as abs difference)
                ...
                # Compute simple absolute difference (no analyzer needed)
                ...
                contrastive_input = torch.cat([
                    ...
                    difference_combined      # Simple difference features
                ], dim=-1)
            ...
            # This path also used during training if original video not provided
                ...
                print(f"[DEBUG-PRE-PROJ] combined_features before projection: {combined_features.shape}")
                combined_features = self.combined_projection(combined_features)
                print(f"[DEBUG-POST-PROJ] combined_features after projection: {combined_features.shape}")
            ...
            if self.enable_explainability:
                # Collect ALL explainability features and component contributions into a single flat vector
                device = next(self.parameters()).device
                all_raw_features = []
                
                # Flatten explainability_features
                for feature in explainability_features:
                    ...
                    # Ensure batch dim
                    if feature.dim() == 0:
                        feature = feature.unsqueeze(0).unsqueeze(0).expand(batch_size, -1)
                    elif feature.dim() == 1:
                        feature = feature.unsqueeze(0).expand(batch_size, -1) if feature.shape[0] != batch_size else feature.unsqueeze(1)
                    elif feature.dim() > 2:
                    ...
                    if feature.shape[0] != batch_size:
                        if feature.shape[0] == 1:
                            ...
                            padding = torch.zeros(batch_size - feature.shape[0], feature.shape[1], device=device)
                            feature = torch.cat([feature, padding], dim=0)
                    all_raw_features.append(feature)
                
                # Flatten weighted component contributions  
                component_weights_norm = F.softmax(self.component_weights, dim=0) if len(self.component_weights) > 0 else None
                for i, (key, value) in enumerate(component_contributions.items()):
                    if value is None or not isinstance(value, torch.Tensor):
                        continue
                    feat = value.view(value.shape[0], -1) if value.dim() > 2 else value
                    if feat.dim() == 1:
                        feat = feat.unsqueeze(1) if feat.shape[0] == batch_size else feat.unsqueeze(0).expand(batch_size, -1)
                    # Fix batch size
                    if feat.shape[0] != batch_size:
                        if feat.shape[0] == 1:
                            ...
                            padding = torch.zeros(batch_size - feat.shape[0], feat.shape[-1], device=device)
                            feat = torch.cat([feat, padding], dim=0)
                    # Apply learned component weight
                    if component_weights_norm is not None and i < len(component_weights_norm):
                        feat = feat * component_weights_norm[i]
                    all_raw_features.append(feat)
                
                # Concatenate all raw features and pad/truncate to fixed 4096 dims
                if all_raw_features:
                    raw_concat = torch.cat(all_raw_features, dim=-1)  # [B, variable_dim]
                    fixed_dim = 4096
                    if raw_concat.shape[-1] < fixed_dim:
                        padding = torch.zeros(batch_size, fixed_dim - raw_concat.shape[-1], device=device)
                        raw_concat = torch.cat([raw_concat, padding], dim=-1)
                    elif raw_concat.shape[-1] > fixed_dim:
                        raw_concat = raw_concat[:, :fixed_dim]
                    
                    # Project to fixed dimension through learned MLP
                    explainability_projected = self.explainability_projector(raw_concat)
                else:
                    explainability_projected = torch.zeros(batch_size, self.expected_classifier_dim - transformer_output.shape[-1], device=device)
                
                # Combine with transformer output — dimensions now ALWAYS match combined_dim
                final_features = torch.cat([transformer_output, explainability_projected], dim=-1)
            else:
                final_features = transformer_output
            
            # Dimensions are now guaranteed to match combined_dim via the explainability_projector
            # No runtime feature_adapter creation needed
            ...
            # 🆕 PER-MODALITY PREDICTIONS: Always use absolute features in deployment mode
            # Deployment mode is now default - model learns from single videos, not pairs
            
            # In deployment mode, we NEVER use difference features (no originals available)
            audio_diff = None
            video_diff = None
            
            # Skip contrastive learning in deployment mode
            if not self.deployment_mode and original_audio_features is not None:
                # TRAINING MODE ONLY: Learn from fake-real difference
                audio_diff = audio_features - original_audio_features
                if self.debug:
                    print(f"[CONTRASTIVE] Audio difference computed: {audio_diff.shape}")
                    print(f"   Mean diff: {audio_diff.abs().mean().item():.4f}")
            
            if not self.deployment_mode and original_video_features is not None:
                # TRAINING MODE ONLY: Learn from fake-real difference  
                video_diff = video_features - original_video_features
                if self.debug:
                    print(f"[CONTRASTIVE] Video difference computed: {video_diff.shape}")
                    print(f"   Mean diff: {video_diff.abs().mean().item():.4f}")
            
            # Audio-only classifier (uses difference if available, else absolute features)
            if audio_diff is not None:
                # Training with pairs: Use learned difference patterns
                audio_only_logits = self.audio_only_classifier(audio_diff)
                if self.debug:
                    print(f"[MODALITY] Audio classifier using DIFFERENCE features (contrastive)")
            else:
                # Inference without pairs: Use absolute features
                audio_only_logits = self.audio_only_classifier(audio_features)
                if self.debug:
                    print(f"[MODALITY] Audio classifier using ABSOLUTE features (single input)")
            
            # Video-only classifier (uses difference if available, else absolute features)
            if video_diff is not None:
                # Training with pairs: Use learned difference patterns
                video_only_logits = self.video_only_classifier(video_diff)
                if self.debug:
                    print(f"[MODALITY] Video classifier using DIFFERENCE features (contrastive)")
            else:
                # Inference without pairs: Use absolute features
                video_only_logits = self.video_only_classifier(video_features)
                if self.debug:
                    print(f"[MODALITY] Video classifier using ABSOLUTE features (single input)")
            
            if self.debug:
                print(f"[MODALITY] Combined logits: {output.shape}")
                print(f"[MODALITY] Audio-only logits: {audio_only_logits.shape}")
                print(f"[MODALITY] Video-only logits: {video_only_logits.shape}")
            
            # ✅ DEPLOYMENT MODE: No batch doubling, no original video processing
            # In deployment mode (default), we NEVER process original videos
            # This prevents shape mismatches and data leakage
            has_originals = False  # Always False in deployment mode
            
            if not self.deployment_mode and original_video_frames is not None and original_video_frames.shape[0] == batch_size:
                # TRAINING MODE ONLY: Process original videos (NOT RECOMMENDED)
                if self.debug:
                    print(f"[WARNING] Processing original videos in training mode - may cause overfitting!")
                    print(f"[WARNING] Consider using deployment_mode=True to disable contrastive learning")
                has_originals = True
            
            # Verify output shape (should always match batch_size in deployment mode)
            if self.debug:
                expected_size = batch_size  # No doubling in deployment mode
                print(f"[DEBUG] Final classifier output shape: {output.shape}")
                print(f"[DEBUG] Expected batch size: {expected_size}")
                if output.shape[0] != expected_size:
                    print(f"[ERROR] Batch size mismatch! Expected {expected_size}, got {output.shape[0]}")
            ...
            results.update({
                'logits': output,
                'audio_only_logits': audio_only_logits,  # 🆕 NEW: Audio tampering detection
                'video_only_logits': video_only_logits,  # 🆕 NEW: Video tampering detection
                'modality_info': {  # 🆕 NEW: Track which modalities were provided
                    'has_video': has_video,
                    'has_audio': has_audio,
                    'mode': 'combined' if (has_video and has_audio) else ('video_only' if has_video else 'audio_only')
                },
                ...
                'detailed_results': results.get('detailed_results', {}) if isinstance(results, dict) else {},  # FIX: Type check
