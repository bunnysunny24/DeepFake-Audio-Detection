...
- Uses deployment components (contrastive learning disabled)
...
# Import model (correct class name)
...
class DeepfakeDetector:
    ...
    def __init__(
        ...
        print(f"[INFO] Initializing DeepfakeDetector on {self.device}")
    ...
    def _load_model(self, checkpoint_path: str) -> nn.Module:
        ...
        # Initialize model architecture matching the training configuration
        model = MultiModalDeepfakeModel(
            ...
            debug=self.debug,
            deployment_mode=True  # Disable contrastive learning for inference
        )
        ...
        if 'model_state_dict' in checkpoint:
            ...
            # Filter out unexpected keys (e.g. contrastive modules not present in deployment)
            model_keys = set(model.state_dict().keys())
            filtered = {k: v for k, v in state_dict.items() if k in model_keys}
            missing = model_keys - set(filtered.keys())
            if missing and self.debug:
                print(f"[DEBUG] {len(missing)} missing keys (deployment mode strips some modules)")
    ...
    def preprocess_video(self, video_path: str) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            frame_indices = np.linspace(0, max(0, total_frames - 1), self.target_frames, dtype=int)
            frames = []
            ...
            # Always release the capture outside the frame loop
            if cap is not None:
                try:
                    cap.release()
                except Exception:
    ...
    def detect_from_video_file(self, video_path: str) -> Dict:
        ...
        Returns:
            results: dict with prediction, confidence, probabilities, etc.
        """
        ...
        with torch.no_grad():
            outputs = self.model(
                ...
                original_audio_waveform=None,  # deployment mode
    ...
    def detect_from_webcam(self, duration: int = 10, display: bool = True):
        ...
        try:
            while (time.time() - start_time) < duration:
                ...
                # Process every target_frames frames
                if len(frame_buffer) >= self.target_frames:
                    ...
                    video_tensor = torch.cat(frames_tensor, dim=0).unsqueeze(0)
                    video_tensor = video_tensor.to(self.device)
...
def main():
    ...
    parser.add_argument('--device', type=str, default=None,
                       choices=['cuda', 'cpu'], help='Device to use (auto-detect if omitted)')
