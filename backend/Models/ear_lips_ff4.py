...
# Create necessary directories for validation/fake only
os.makedirs(os.path.join(ear_dir, "validation", "fake"), exist_ok=True)
os.makedirs(os.path.join(optical_flow_dir, "validation", "fake"), exist_ok=True)

# Function to generate frames (ONLY for validation/fake)
def generate_validation_fake_frames():
    input_folder = os.path.join(image_dir, "validation", "fake")
    ear_output_folder = os.path.join(ear_dir, "validation", "fake")
    optical_output_folder = os.path.join(optical_flow_dir, "validation", "fake")

    for file_name in tqdm(os.listdir(input_folder), desc="Processing Validation/Fake Images"):
...
generate_validation_fake_frames()

print("✅ Validation/Fake Ear and Optical Flow frames generated successfully!")
