...
# Create necessary directories for train/real only
os.makedirs(os.path.join(ear_dir, "train", "real"), exist_ok=True)
os.makedirs(os.path.join(optical_flow_dir, "train", "real"), exist_ok=True)

# Function to generate frames (ONLY for train/real)
def generate_train_real_frames():
    input_folder = os.path.join(image_dir, "train", "real")
    ear_output_folder = os.path.join(ear_dir, "train", "real")
    optical_output_folder = os.path.join(optical_flow_dir, "train", "real")

    for file_name in tqdm(os.listdir(input_folder), desc="Processing Train/Real Images"):
...
# Process only train/real images
generate_train_real_frames()

print("✅ Train/Real Ear and Optical Flow frames generated successfully!")
