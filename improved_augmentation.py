...
# NOTE: Do NOT set global random seeds here - it kills augmentation diversity
# Set seeds in the main training script instead via set_seed()
...
def get_advanced_video_transforms(train=True, use_production_robust=True):
    ...
    if train:
        ...
        # Validation transforms - ALWAYS use clean, deterministic transforms
        # No random augmentation during validation to ensure consistent metrics
        return A.Compose([
            A.Resize(height=224, width=224),
            A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            ToTensorV2(),
